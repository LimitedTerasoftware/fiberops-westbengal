...
class EmployeeController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $this->data['visitors'] = VisitingDetails::where('employee_id','=',$id)->whereMonth('created_at', date('m'))->orderBy('id', 'DESC')->get();
        $ids=$id;

      $this->data['ids']   = $ids;
        return view('admin.employee.show1', $this->data);
    }
    ...
    public function show1(Request $request,$id)
    {
        ...
        if ($_POST) {
            ...
            if ($request->from_date=='' && $request->employee_id=='') {
                $this->data['visitors'] = VisitingDetails::orderBy('id', 'DESC')->where('employee_id','=',$id)->get(); 
              }else{
            $this->data['visitors'] = VisitingDetails::whereBetween('created_at', [$dateBetween['from_date'], $dateBetween['to_date']])->where('employee_id','=',$id)->orderBy('id', 'DESC')->get();
              }
        ...
        return Datatables::of($employeeArray)
            ->addColumn('action', function ($employee) {
                ...
                if (auth()->user()->can('employees_delete')) {
                    $retAction .= '<form class="float-left pl-2" action="' . route('admin.employees.edit', $employee) . '" method="POST">' . method_field('DELETE') . csrf_field() . '<button class="btn btn-sm btn-icon btn-danger" data-toggle="tooltip" data-placement="top" title="Delete"> <i class="fa fa-trash"></i></button></form>';
                }

                return $retAction;
            })
            ->addColumn('image', function ($employee) {
                return '<figure class="avatar mr-2"><img src="' .asset('images/img/'.$employee->img) . '" alt=""></figure>';
            })
    ...
    public function getVisitor(Request $request,$id)
    {
