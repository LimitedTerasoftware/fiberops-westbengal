...
class VisitorController extends Controller
{
    ...
    public function __construct(VisitorService $visitorService)
    {
        ...
        $this->middleware(['permission:visitors_filter'])->only('filter');
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.visitor.index',$this->data);
    }
    ...
    public function index1(Request $request)
    {
        ...
        return view('admin.visitor.index1',$this->data);
    }

    public function create(Request $request)
    {
        $sid = session('login_web_59ba36addc2b2f9401580f014c7f58ea4e30989d');
      
        $this->data['centers'] = Employee::where('status', Status::ACTIVE)->where('user_id','=', $sid)->get();
        $this->data['employees'] = Employee::where('status', Status::ACTIVE)->get();
        $this->data['contensts'] = DB::table('usedcontents')->get();
     ...
     public function search(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'visitorID.required' => 'Visitor Phone Number required',
            'visitorID.numeric' => 'Phone Number must be numeric'
        ]);
        ...
        $v_id=DB::table("visiting_details as vd")->join("visitors as v", function($join){
            $join->on("vd.visitor_id", "=", "v.id");
            })->where("v.phone", "=", $id)
               ->whereNull("vd.checkout_at")->latest('vd.created_at')->first();
        if(!$v_id){
                     $v_id1=Visitor::where('phone','=',$id)->latest()->first();
                        if(!$v_id1){
                                    $v='00000';
                                    } else{
                                     $v=$v_id1->id;
        ...
        $visitingDetail = VisitingDetails::where('visitor_id', $v)->first();

        if(!$visitingDetail){
            $check_in =  date('y-m-d H:i');
        }else{
            $check_in =  $visitingDetail->checkin_at;
        }
        
        $checkout_at = date('y-m-d H:i');
        $seconddiff = (strtotime($checkout_at) - strtotime($check_in));
       ...
       $hours1 = sprintf("%02d", $hours);
       $minutes1 = sprintf("%02d", $minutes);
       $seconds1 = sprintf("%02d", $seconds);

        $duration= "$hours1:$minutes1:$seconds1";
       
        if ($visitingDetail && (!$visitingDetail->checkout_at)) {
            $visitingDetail->checkout_at = date('y-m-d H:i');
            $visitingDetail->time_duration = $duration;
            $visitingDetail->save();
        ...
        return Datatables::of($visitingDetailArray)
            ->addColumn('action', function ($visitingDetail) {
                ...
                if (auth()->user()->can('visitors_show')) {
                    $retAction .= '<a href="' . route('admin.visitors.show', $visitingDetail) . '" class="btn btn-sm btn-icon mr-1  float-left btn-info" data-toggle="tooltip" data-placement="top" title="User details"><i class="far fa-eye"></i></a>';
                }
    ...
    public function getVisitor1(Request $request)
    {
        
        $visitingDetails = $this->visitorService->all1();
        $i            = 1;
        $visitingDetailArray = [];
        if (!blank($visitingDetails)) {
            foreach ($visitingDetails as $visitingDetail) {
