...
//
    ...
    public function index(Request $request)
    {
        ...
        $attendances = DB::table('attendances')->join('users', 'users.id', '=', 'attendances.user_id')
        ->whereDate('attendances.created_at', date('y-m-d'))
        ->orderBy('id', 'desc')
        ->select('attendances.*', 'users.img','users.first_name','users.last_name',)
        ->get();


        return view('admin.attendance.index1',compact('attendance','attendances'));
    }
    ...
    {
        $attendances = Attendance::whereDate('created_at', date('y-m-d'))->orderBy('id', 'desc')->get();
        $i            = 1;
        $attendanceArray = [];
