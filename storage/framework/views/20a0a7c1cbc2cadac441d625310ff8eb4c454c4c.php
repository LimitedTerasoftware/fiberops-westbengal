<?php $__env->startSection('title', 'Workforce Dashboard - '); ?>
    ...
    <div class="container-fluid">
       ...
       <!-- Statistics Cards -->
        <div class="stats-grid">
            <a href="<?php echo e(route('admin.attendance_list', $queryParams)); ?>" class="stat-card">
            ...
            <a href="<?php echo e(route('admin.attendance_list', array_merge(request()->all(), ['status' => 'present']))); ?>" class="stat-card">
            ...
            <a href="<?php echo e(route('admin.workforce_details', array_merge(request()->all(), ['stage' => 'no_ticket']))); ?>" class="stat-card">
                <div class="stat-content">
            ...
            <a href="<?php echo e(route('admin.attendance_list', array_merge(request()->all(), ['status' => 'absent']))); ?>" class="stat-card">
                <div class="stat-content">
            ...
            <a href="<?php echo e(route('admin.workforce_details', array_merge(request()->all(), ['stage' => 'working']))); ?>" class="stat-card">
           ...
           <a href="<?php echo e(route('admin.workforce_details', array_merge(request()->all(), ['stage' => 'only_hold']))); ?>" class="stat-card">
           ...
           <a href="<?php echo e(route('admin.workforce_details', array_merge(request()->all(), ['stage' => 'completed']))); ?>" class="stat-card">
           ...
           <a href="<?php echo e(route('admin.workforce_details', array_merge(request()->all(), ['stage' => 'not_started']))); ?>" class="stat-card">
        ...
        <div class="canvas-card mt-4">
            <div class="heatmap-header"
                ...
                <div  style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <input type="date" id="frt_from_date" class="form-control form-control-sm px-1"
                        style="width: 105px; font-size: 12px;border-radius: 4px;">
                    <label class="small fw-bold me-1">To:</label>
                    <input type="date" id="frt_to_date" class="form-control form-control-sm px-1"
                        style="width: 105px; font-size: 12px;border-radius: 4px;">
                    <button class="btn btn-primary btn-sm px-2 py-0 mx-1" id="btn-frt-filter"
                        style="font-size: 12px; border-radius: 4px;">Go</button>
                </div>
            </div>
            <div id="frtCompletionHeatmap" class="heatmap"></div>
        </div>
    </div>
        ...
        <div class="canvas-card mt-4">
            <div class="heatmap-header"
                ...
                <div  style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <label class="small fw-bold me-1">From:</label>
                    <input type="date" id="pat_from_date" class="form-control form-control-sm px-1"
                        style="width: 105px; font-size: 12px;border-radius: 4px;">
                    <label class="small fw-bold me-1">To:</label>
                    <input type="date" id="pat_to_date" class="form-control form-control-sm px-1"
                        style="width: 105px; font-size: 12px;border-radius: 4px;">
                    <button class="btn btn-primary btn-sm px-2 py-0 mx-1" id="btn-pat-filter"
                        style="font-size: 12px; border-radius: 4px;">Go</button>
                </div>
            </div>
            <div id="patCompletionHeatmap" class="heatmap"></div>
        </div>
    </div>
        ...
        <div class="canvas-card mt-4">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                ...
                <div class="gap-1"  style="display:flex; justify-content:space-between; align-items:center;">
                    <label class="small fw-bold me-1">From:</label>
                    <input type="date" id="trend_from_date" class="form-control form-control-sm px-1"
                        style="width: 105px; font-size: 12px; border-radius: 4px;">
                    <label class="small fw-bold me-1">To:</label>
                    <input type="date" id="trend_to_date" class="form-control form-control-sm px-1"
                        style="width: 105px; font-size: 12px;border-radius: 4px;">
                    <button class="btn btn-primary btn-sm px-2 py-0 mx-1" id="btn-trend-filter"
                        style="font-size: 12px;border-radius: 4px;">Go</button>
                </div>
            </div>
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
   <script>
            ...
            function fetchCompletionTrend() {
                const fromDate = $('#trend_from_date').val();
                const toDate = $('#trend_to_date').val();

                $.ajax({
                    url: "<?php echo e(route('admin.get_completion_trend')); ?>",
                    method: "GET",
                    data: {
                        from_date: fromDate,
                        to_date: toDate
                    ...
                    success: function (response) {
                        renderCompletionTrendChart(response);
                    },
                    error: function (err) {
            ...
            function fetchRecurringGpTrends() {
                    ...
                    $.ajax({
                        url: "<?php echo e(route('admin.get_recurring_gp_trends')); ?>",
                        method: "GET",
                        data: {
                            from_date: fromDate,
                            to_date: toDate,
      ...
      function renderRecurringGpsChart(data) {
            ...
            function renderCompletionTrendChart(data) {
                ...
                const frtAssigned = data.map(item => item.frt_assigned);
                const frtCompleted = data.map(item => item.frt_completed);
                const patAssigned = data.map(item => item.pat_assigned);
                const patCompleted = data.map(item => item.pat_completed);
...
$(document).ready(function() {
    $.ajax({
        url: "<?php echo e(url('/admin/get_todayfrtreport')); ?>",// Your Laravel route
        method: 'GET',
        dataType: 'json',
        success: function(response) {
            let frtTotal = 0, frtWorking = 0, frtNotStarted = 0, frtCompleted = 0;
            let frtNoTicket = 0, frtOnlyHold = 0, frtLoggedIn = 0, frtNotLoggedIn = 0, frtLeave =0;
            let frtOnline = 0, frtOffline = 0; 

            let patTotal = 0, patWorking = 0, patNotStarted = 0, patCompleted = 0;
            let patNoTicket = 0, patOnlyHold = 0, patLoggedIn = 0, patNotLoggedIn = 0 ,patLeave=0;
            ...
            $.each(response.zones, function(zoneId, zoneData) {
                ...
                patLeave +=zoneData.patrollers.leave;
            ...
            let NotLoggedIn = frtNotLoggedIn+patNotLoggedIn+patLeave+frtLeave;
            let Leave =patLeave+frtLeave;
...
function renderCompletionHeatmap(data, type,fromDate=null,toDate=null) {
        const containerId = type === 'frt' ? '#frtCompletionHeatmap' : '#patCompletionHeatmap';
        const heatmapContainer = $(containerId);
        heatmapContainer.empty();

        const stages = ['completed_0_4', 'completed_4_10', 'completed_10_24', 'completed_24_48', 'completed_gt_48'];
        const stageLabels = ['0-4 Hrs', '4-10 Hrs', '10-24 Hrs', '24-48 Hrs', '> 48 Hrs'];
        ...
        // === Zone Rows ===
        $.each(data.zones, function (zoneId, zoneData) {
            const groupData = type === 'frt' ? zoneData.frt : zoneData.patrollers; // dynamic access
            const zoneName = zoneData.zone_name;
            const total = (groupData.completed_0_4 + groupData.completed_4_10 + groupData.completed_10_24 + groupData.completed_24_48 + groupData.completed_gt_48) || 1;

            let row = `<div class="heatmap-row">
                <div class="zone-name">${zoneName}</div>`;

            stages.forEach((stage, i) => {
                const value = groupData[stage] ?? 0;
                // const percent = (value / total) * 100;
                ...
                if (value > 0) {
                    ...
                    if (stage === 'completed_0_4') riskClass = 'good';
                    else if (stage === 'completed_4_10') riskClass = 'good'; // acceptable
                    else if (stage === 'completed_10_24') riskClass = 'medium'; // warning
                    else if (stage === 'completed_24_48') riskClass = 'bad';   // bad
                    else if (stage === 'completed_gt_48') riskClass = 'bad';   // bad
                }
                 let dateParams = '';
                        if (fromDate && toDate) {
                  ...
                  const url = `/admin/workforce_details?zone_id=${zoneData.zone_id}&stage=${stage}&type=${type}${dateParams}`;
                        row += `<a href="${url}" class="cell ${riskClass}" title="${stageLabels[i]}: ${value}">
                                                    ${value}
                                                  </a>`;
            });
...
const frtTotCount = data.frt.total;
...
new Chart(frtCtx, {
    ...
    options: {
        plugins: {
            title: { 
                display: true, 
                text: `FRT Work Progress (Total: ${frtTotCount} / Working:${frtTotal})`, // ?? show total count in title
                font: { size: 14, weight: 'bold' },
                align: 'start',
                padding: { bottom: 10 }
...
const patTotCount = data.pat.total;
...
new Chart(patCtx, {
    ...
    options: {
        plugins: {
            title: { 
                display: true, 
                text: `Patrollers Work Progress (Total:${patTotCount} / Working:${patTotal})`,
                font: { size: 14, weight: 'bold' },
                align: 'start',
                padding: { bottom: 10 }
            },
...
const patLabels = [
    ...
    `Patroller Leave(${data.pat.leave})`,
];
...
// Combined data array
const finalData = [
    data.frt.logged_in, data.frt.not_logged_in, data.frt.offline,data.frt.leave,
    data.pat.logged_in, data.pat.not_logged_in, data.pat.offline,data.pat.leave,
];

// Matching colors
const finalColors = [
    '#22c55e', '#ef4444', '#9ca3af', '#f60da5', // FRT
    '#3b82f6', '#f87171', '#d1d5db' ,'#c761a3' ,  // Patrollers
];
...
$(document).ready(function () {
    ...
    // LOAD DEFAULT (ALL)
    $.ajax({
        url: "<?php echo e(url('/admin/ongoing_ticket_data')); ?>",
        method: "GET",
        dataType: "json",
...
function loadHeatmapWithFilter(filterType) {
    ...
    $.ajax({
        ...
        error: function (xhr) {
            console.error("Filter API error:", xhr.responseText);
              ...
              $(document).ready(function () {
                ...
                $('#frt_from_date').val(today);
                $('#frt_to_date').val(today);
                $('#pat_from_date').val(today);
                $('#pat_to_date').val(today);

                // FRT Filter Click Handler
                $('#btn-frt-filter').on('click', function () {
                    const fromDate = $('#frt_from_date').val();
                    const toDate = $('#frt_to_date').val();
                    if (fromDate && toDate) {
                        loadCompletionData(fromDate, toDate, 'frt');
                    } else {
                        alert('Please select both From and To dates');
                    }
                });

                // Patroller Filter Click Handler
                $('#btn-pat-filter').on('click', function () {
                    const fromDate = $('#pat_from_date').val();
                    const toDate = $('#pat_to_date').val();
                    if (fromDate && toDate) {
                        loadCompletionData(fromDate, toDate, 'patrollers');
                    } else {
                        alert('Please select both From and To dates');
                    }
                });
        ...
        function loadCompletionData(fromDate, toDate, type) {
                ...
                $.ajax({
                    url: "<?php echo e(route('admin.get_todayfrtreport')); ?>",
                    method: "GET",
                    data: { from_date: fromDate, to_date: toDate },
                    success: function (response) {
                        console.log(`Completion data loaded for ${type}:`, response);
                        // Only render the requested heatmap type
                        renderCompletionHeatmap(response, type, fromDate, toDate);
                    },
                    error: function (err) {
