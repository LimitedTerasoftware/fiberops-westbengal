    ...
    <div class="page-header"> 
        <div class="header-content">
             <button class="close-btn" onclick="history.back()"> 
                ...
                <h1 class="page-title">Ongoing Ticket Tracking History</h1> 
            </div> 
        </div>
    ...
    <div class="left-sidebar">
        ...
        <div class="employee-card">
            ...
            <div class="ticket-info">
                <span class=""><b>Ticket Details:</b></span>
                <span class="">{{ $ticket->booking_id}} ({{ $ticket->autoclose}})</span>
                <span class="">{{ $ticket->gpname}} / {{ $ticket->mandal}} / {{ $ticket->district}}</span>
                <span class="">{{ $ticket->downreason}} / {{ $ticket->subcategory}}</span>
                <span class="">Down : {{ $ticket->downdate}} {{ $ticket->downtime}}</span>
                <span class="">Strated : {{ $ticket->started_at}}</span>
...
function loadTrackingData() {
    ...
    const url = `/admin/track_data/${providerId}`;
...
function displayTrackingOnMap() {
    ...
    roadDistances = {
        total: roadDistances.total,
        daily: roadDistances.daily,
    ...
    dates.forEach((date, dayIndex) => {
        ...
        //     datetime: p.datetime
        // }));
        const formattedPoints = dayPoints
            .map(p => ({ lat: parseFloat(p.latitude), lng: parseFloat(p.longitude), datetime: p.datetime }))
            .filter(p => !isNaN(p.lat) && !isNaN(p.lng));
        if (formattedPoints.length < 2) {
            console.warn(`Not enough valid points for ${date}`);
            routesLoadedCount++;
            return;
        }
            ...
            (response, status) => {
                if (status === google.maps.DirectionsStatus.OK) {
                    ...
                    // addPolylineHoverEffect(polyline, route, dayColor, date, dayPoints.length);
                     addPolylineHoverEffect(polyline, formattedPoints, dayColor, date, roadDistances.daily[date]);
                    ...
                    console.error(`Directions request failed for ${date}: ${status}`);
                     const path = formattedPoints.map(p => ({ lat: p.lat, lng: p.lng }));
                    const polyline = new google.maps.Polyline({
                        path: path,
                        geodesic: true,
                        strokeColor: dayColor,
                    ...
                    addMarkerInfoWindow(new google.maps.Marker({ position: startPoint, map: trackingMap, icon: { path: google.maps.SymbolPath.CIRCLE, scale: 10, fillColor: '#10b981', fillOpacity: 1, strokeWeight: 3, strokeColor: '#fff' }, title: 'Start Point' }), 'Start Point', dayPoints[0].datetime, dayPoints[0].address);
                    addMarkerInfoWindow(new google.maps.Marker({ position: endPoint, map: trackingMap, icon: { path: google.maps.SymbolPath.CIRCLE, scale: 10, fillColor: '#ef4444', fillOpacity: 1, strokeWeight: 3, strokeColor: '#fff' }, title: 'End Point' }), 'End Point', dayPoints[dayPoints.length - 1].datetime, dayPoints[dayPoints.length - 1].address);
...
function addPolylineHoverEffect(polyline, points, color, date, dayDistance) {
    // Pre-calculate straight-line distances between points
    ...
    const totalStraightLine = cumulativeDistances[cumulativeDistances.length - 1];

    // Scale factor to match PHP total distance
    const scaleFactor = totalStraightLine > 0 ? dayDistance / totalStraightLine : 1;

    // Apply scale factor
    cumulativeDistances = cumulativeDistances.map(d => d * scaleFactor);
    ...
    google.maps.event.addListener(polyline, 'mousemove', function(event) {
        ...
        const distanceKm = cumulativeDistances[closestIndex];
