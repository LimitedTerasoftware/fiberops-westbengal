        ...
        <div class="filter-pill">
          ...
          <div class="filter-pill">
            ...
            <select name="status" id="status">
                ...
                <option value="only_hold">Hold</option>
                <option value="no_ticket">No Tickets</option>
            </select>
        </div>
...
$(document).ready(function() {
    ...
    function loadFrtDetails() {
        ...
        let stage       = firstLoad ? (urlParams.stage || '') : $('#status').val();
        ...
        let filter      = firstLoad ?  urlParams.filter : '';
        ...
        const apiUrl = "{{ route('admin.get_todayfrt_details') }}" + '?' + $.param(filters);
                ...
                $('#frtTable').DataTable({
                   ...
                   { extend: 'print', text: 'Print' }
                    ...
                    columns: [
                        ...
                        { data: 'pickup_tickets', title: 'Ongoing',
                           render: { _: data => parseInt(data) || 0,
                           display: function (data, type, row) {
                                 let value = parseInt(data) || 0;
                                 let color = value === 0 ? "red" : value <= 1 ? "orange" : "green";
                                 let url = `{{ url('admin/tickets') }}?provider_id=${row.provider_id}&status=OnGoing&from_date=${fromDate}&to_date=${toDate}`;
                                        return `<a href="${url}" target="_blank" style="color:${color}; font-weight:bold;">${value}</a>`;
                                 }
                              }
                        ...
                        // },
                         {
                                ...
                                render: {
                                    ...
                                    display: function (data, type, row) {
                                        ...
                                        const currentStage = urlParams.get('stage') || $('#status').val();
                                        
                                        if (currentStage === 'completed_0_4') intervalParam = '&interval=0-4';
                                        else if (currentStage === 'completed_4_10') intervalParam = '&interval=4-10';
                                        else if (currentStage === 'completed_10_24') intervalParam = '&interval=10-24';
                                        else if (currentStage === 'completed_24_48') intervalParam = '&interval=24-48';
                                        else if (currentStage === 'completed_gt_48') intervalParam = '&interval=>48';
                                        let url = `{{ url('admin/tickets') }}?provider_id=${row.provider_id}&status=Completed&searchinfo=Manual&from_date=${fromDate}&to_date=${toDate}${intervalParam}`;
                                        return `<a href="${url}" target="_blank" style="color:${color}; font-weight:bold;">${value}</a>`;
                                 }
                              }
                        },
