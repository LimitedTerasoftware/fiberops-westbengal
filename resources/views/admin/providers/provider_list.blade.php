...
<div class="content-area py-1">
    <div class="container-fluid">
        ...
        <div class="header-section mb-1">
            <div class="header-content">
             ...
             <button class="btn btn-export" id="">
                <i i class="fa fa-plus me-2"></i> New
            </button>
            </a>&nbsp;&nbsp;
    ...
    <form method="GET" action="{{ route('admin.provider.index') }}">

        <div class="filters-grid">
              <div class="filter-group">
               
                 <label class="filter-label">Search</label>
                        <input type="text" name="search" value="{{ request('search') }}" class="filter-input" placeholder="Name, Email, Phone...">
            ...
            <div class="filter-group">
             ...
             <button type="submit" class="action-btn action-btn-primary">Apply</button>
                        <a href="{{ route('admin.provider.index') }}" class="action-btn action-btn-secondary">Clear</a>
        ...
        <div class="table-section">
            <h2 class="table-title">Detailed Technicians List</h2>
                ...
                <table class="attendance-table">
                    <thead>
                        <tr>
                            ...
                            <th class="th-zone">District</th>
                            <th class="th-zone">Version</th>
                            <th class="th-km">Date of Joining</th>
                            ...
                            <tr class="table-row">
                                <td class="td-staff">
                                    <div class="staff-info">
                                        ...
                                        <img src="{{ !empty($provider->online_image) ? asset($provider->online_image) : 'https://cdn-icons-png.flaticon.com/512/847/847969.png' }}"
    `                                 alt="{{ $provider->first_name }} {{ $provider->last_name }}">
                                        ...
                                        <div class="staff-details">
                                            ...
                                            <div class="staff-id">{{ $provider->email}}</div>
                                ...
                                <td class="td-district">{{ isset($provider->district_name) ? $provider->district_name : 'N/A' }}</td>
                                <td class="td-verstion">{{ isset($provider->version) ? $provider->version : 'N/A' }}</td>

                                <td class="td-km">{{ isset($provider->joiningdate) ? \Carbon\Carbon::parse($provider->joiningdate)->format('d-m-Y') :'-'}}</td>
                                <td class="td-status">
                                ...
                                <td class="td-status">
                                    @if($provider->leave_status == "On Leave")
                                        <span class="status-badge status-late">On Leave</span>
                                    @else
                               ...
                               <button class="action-btn action-btn-danger" onclick="AddLeave({{ $provider->id }}, '{{ $provider->first_name }} {{ $provider->last_name }}')">
                                     <i class="ti-plus"></i>
                                </button>
                                 ...
                                 @if(auth()->user()->role == 'admin')  
                                    ...
                                    <button class="action-btn action-btn-primary">
                                        <a href="{{ route('admin.staff_livetrack', $provider->id) }}?date={{ date('Y-m-d') }}">
                                            <!-- View Route -->
                                    ...
                                    <button class="action-btn action-btn-secondary">
                                        <a href="{{ url('admin/staff_details/' . $provider->id) }}" >
                                             <i class="fa fa-eye"></i>
                                        </a>       
                                    </button>
                                    <button class="action-btn action-btn-secondary">
            ...
            <div class="terrasoft-modal-container" onclick="event.stopPropagation()">
                ...
                <div class="terrasoft-modal-header">
                    <div class="terrasoft-modal-title-section">
                        ...
                        <h3 class="terrasoft-modal-title">Add New Leave</h3>
                ...
                <form id="addLeaveForm" class="terrasoft-modal-form" onsubmit="submitLeaveForm(event)">
                    ...
                    <div class="terrasoft-modal-body">
                        ...
                        <input type="hidden" id="provider_id" name="provider_id" value="">
                         <input type="hidden" id="type" name="type" value="leave">
                        {{-- Date Fields Row --}}
                        <div class="terrasoft-form-row">
                            <div class="terrasoft-form-group">
                                <label for="start_date" class="terrasoft-form-label">
                                    Start Date <span class="terrasoft-required">*</span>
                                </label>
                                <input type="date" 
                                    id="start_date" 
                                    name="start_date" 
                            ...
                            <div class="terrasoft-form-group">
                                <label for="end_date" class="terrasoft-form-label">
                                    End Date <span class="terrasoft-required">*</span>
                                </label>
                                <input type="date" 
                                    id="end_date" 
                                    name="end_date" 
                        ...
                        {{-- Reason Field --}}
                        <div class="terrasoft-form-group">
                            <label for="reason" class="terrasoft-form-label">
                            ...
                            <textarea id="reason" 
                                    ...
                                    rows="3" 
                                    placeholder="Enter the reason for leave..."></textarea>
                        </div>

                        {{-- Status Field --}}
                        <div class="terrasoft-form-group">
                            <label for="status" class="terrasoft-form-label">
                                <i class="ti-check-circle"></i>
                                Status
                            </label>
                            <select id="status" 
                                    name="status" 
                                    class="terrasoft-form-select">
...
.action-btn-danger {
    background-color: transparent;
    border-color: #d40a0aff;
    color: #600210ff;
}
.action-btn-danger:hover {
    background-color: #ef6262ff;
    color: white;
}
...
function AddLeave(employeeId, name) {
