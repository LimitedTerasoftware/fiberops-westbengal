...
<div class="terrasoft-main-content">
    <div class="terrasoft-page-container">
        <div class="terrasoft-page-header">
            <div class="terrasoft-header-content">
                <div class="terrasoft-header-icon">
                    <i class="ti-pencil text-green-600"></i>
                </div>
                <div>
                    <h1 class="terrasoft-page-title">Edit Holiday</h1>
                    <p class="terrasoft-page-subtitle">Update holiday details</p>
                </div>
        ...
        <div class="terrasoft-form-container">
            <form action="{{ route('admin.holidays.update', $holiday->id) }}" method="POST" class="terrasoft-form" id="holidayForm">
                {{ csrf_field() }}
                    ...
                    <div class="terrasoft-form-grid">
                        ...
                        <div class="terrasoft-form-group">
                            ...
                            <input type="date"
                                  ...
                                  value="{{ isset($holiday->date) ? \Carbon\Carbon::parse($holiday->date)->format('Y-m-d') : '' }}"                                   required>
                            @if ($errors->has('date'))
                                <div class="terrasoft-error-message">
                        ...
                        <div class="terrasoft-form-group">
                            ...
                            <select name="type"
                                ...
                                <option value="regional" {{ $holiday->type == 'regional' ? 'selected' : '' }}>Regional</option>
                        ...
                        <div class="terrasoft-form-group">
                            ...
                            <select name="applies_to"
                                    ...
                                    required
                                    onchange="toggleLocationFields({{ $state_id ?? 'null' }})">
                                <option value="all" {{ $holiday->applies_to == 'all' ? 'selected' : '' }}>All</option>
                                <option value="state" {{ $holiday->applies_to == 'state' ? 'selected' : '' }}>State</option>
                                <option value="district" {{ $holiday->applies_to == 'district' ? 'selected' : '' }}>District</option>
                                <option value="block" {{ $holiday->applies_to == 'block' ? 'selected' : '' }}>Block</option>
                            </select>
                        </div>


                        <div class="terrasoft-form-group" id="district_field" style="display:{{ in_array($holiday->applies_to, ['district', 'block']) ? 'block' : 'none' }};">
                            ...
                            <select name="district_id"
                                ...
                                @foreach($districts as $district)
                                    <option value="{{ $district->id }}" {{ $holiday->district_id == $district->id ? 'selected' : '' }}>{{ $district->name }}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="terrasoft-form-group" id="block_field" style="display:{{ $holiday->applies_to == 'block' ? 'block' : 'none' }};">
                            ...
                            <select name="block_id"
                                ...
                                @foreach($blocks as $block)
                                    <option value="{{ $block->id }}" {{ $holiday->block_id == $block->id ? 'selected' : '' }}>{{ $block->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="terrasoft-form-group">
                            ...
                            <select name="duration"
                                    ...
                                    class="terrasoft-form-input"
                                    required>
                                <option value="full" {{ $holiday->duration == 'full' ? 'selected' : '' }}>Full Day</option>
                                <option value="half" {{ $holiday->duration == 'half' ? 'selected' : '' }}>Half Day</option>
                            </select>
                        </div>
                        ...
                        <div class="terrasoft-form-group">
                            ...
                            <div class="terrasoft-checkbox">
                                <input type="checkbox" name="is_recurring" value="1" {{ $holiday->is_recurring ? 'checked' : '' }}>
                    ...
                    <div class="terrasoft-form-grid">
                        <div class="terrasoft-form-group terrasoft-form-group-full">
                            ...
                            <textarea name="description"
                                      ...
                                      rows="3"
                                      placeholder="Enter holiday description...">{{ $holiday->description }}</textarea>
                        </div>
                    </div>
                </div>
...
<script>
let userStateId = {{ $state_id ?? 'null' }};
let selectedDistrictId = '{{ $holiday->district_id }}';
let selectedBlockId = '{{ $holiday->block_id }}';

function toggleLocationFields() {
    var appliesTo = document.getElementById('applies_to').value;
    var stateField = document.getElementById('state_field');
    var districtField = document.getElementById('district_field');
    var blockField = document.getElementById('block_field');
    var stateSelect = document.getElementById('state_id');

    districtField.style.display = (appliesTo == 'district' || appliesTo == 'block') ? '' : 'none';
    blockField.style.display = appliesTo == 'block' ? '' : 'none';
